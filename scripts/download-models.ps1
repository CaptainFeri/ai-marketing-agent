...
.PARAMETER LlmVariant
    Which primary LLM to fetch: qwen3-30b-a3b (default, official unquantized
    weights, ~60GB, needs multi-GPU or an 80GB card), qwen3-30b-a3b-awq
    (community 4-bit AWQ mirror, ~18GB, fits a single 24GB card),
    qwen3-14b or gemma-3-12b (smaller fallbacks per the handoff's model
    table).
...
switch ($LlmVariant) {
    "qwen3-30b-a3b" {
        Get-HfModel -Label "LLM: Qwen3-30B-A3B (official, unquantized bf16, ~60GB)" `
            -Repo "Qwen/Qwen3-30B-A3B" -Dest "llm\qwen3-30b-a3b"
    }
    "qwen3-30b-a3b-awq" {
        Write-Warning "qwen3-30b-a3b-awq is a THIRD-PARTY community quantization, not published by Qwen. Verify it yourself before trusting it in production."
        Get-HfModel -Label "LLM: Qwen3-30B-A3B AWQ (community 4-bit, ~18GB)" `
            -Repo "QuixiAI/Qwen3-30B-A3B-AWQ" -Dest "llm\qwen3-30b-a3b-awq"
    }
    "qwen3-14b" {
